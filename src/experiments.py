...
def regionGrowingWithUnsharpMasking(img):
    # Gaussian blur
...
sigma = fwhm2sigma(5)
print(sigma)
liverData = liverReader.readInterpolatedLiverData(sampleNumber)/255
print(np.max(liverData))
print(np.min(liverData))


roi = liverReader.readInterpolatedLiverData("01")
